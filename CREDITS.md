...
Both tracks are **CC0 (public domain)** from [OpenGameArt](https://opengameart.org), verified on
their own submission pages rather than trusted from a search filter. That check earned its keep: the
first two tracks chosen were **GPL 3.0**, which is copyleft and cannot travel with an MIT repository
or its itch build.

| Where | Track | Author | Source |
|---|---|---|---|
| The sixty-second raid | *Duel of the Dungeons* | **Umplix** | <https://opengameart.org/content/duel-of-the-dungeons> |
| Standings, shop, review, endings | *Within The Abandoned Castle* | **Spring Spring** | <https://opengameart.org/content/within-the-abandoned-castle> |

Neither author requires credit. Both are credited anyway, for the reason given above about the Crawl
tiles.

Both were re-encoded to Vorbis for the web build, and the calm track is a ninety-second excerpt with
two-second fades rather than its full eight and a half minutes: 33 MB of source became 2.1 MB
shipped. CC0 permits the edit outright, and on a jam page a slow load is a lost vote.

## Sound effects

Every other sound is generated at runtime by `SfxSynth` — ten procedural effects, no audio files.
That is why the game had no audio licensing question at all until music arrived.
