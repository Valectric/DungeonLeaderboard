...
> **Read this and skip the addenda unless you are picking the work up.**
>
> **The flip works.** Entrance at the bottom, rooms running up, crossing times matching the horizontal
> dungeon to the decimal. It is on `flip-vertical-wip` and unmerged for one reason: under a *maximal
> ambush* -- a monster spawned at every spawner every tick, which no player can produce -- parties
> that survive going east die going north, and that trips `KillingTheParty_NeverPaysBest`.
>
> **Getting there found four bugs that were already in the code**, all hidden by the dungeon happening
> to run east. Those are fixed and are the flip's real value.
>
> **Ten addenda chase the remaining failure and none closes it.** Six were measuring something
> adjacent to the question. If you pick this up, start at addendum 10: the recording harness spawns
> per frame and the measurement per fixed tick, so they are not the same experiment and no visual
> evidence gathered so far can be read against the numbers. Fix that first -- it is one change --
> before forming a fifth hypothesis.
>
> **And ask whether the test is the right instrument at all.** Addendum 7 found the 434 it compares
> against is a single adventurer pinned at a sliver of health for thirty-four seconds. The rule it
> defends is the one the design rests on; the scenario may not be.
