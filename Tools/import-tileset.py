...
def level(image, gain, lift):
    """
    Darkens a tile without touching its hue.

    The stone in this game is COLD -- the moodboard's violet is the backdrop and its magenta is
    arcane light, not masonry. An earlier pass mapped stone onto the violet ramp and produced a
    dungeon that measured correctly and looked like a sweet shop. So the art goes in as its artist
    drew it and only the levels move: multiplied down to sit against a near-black floor, with a
    small lift so the darkest pixels do not crush to one flat black.
    """
    source = np.asarray(image, dtype=np.float32)
    rgb, alpha = source[..., :3], source[..., 3:4]
    out = np.clip((rgb * gain) + lift, 0, 255)
...
def shape_wall(base, mask):
    ...
    # Sampled from the tile itself, so the lit edge is this stone catching light rather than a
    # colour borrowed from a palette the masonry is not drawn in.
    stone = np.asarray(base, dtype=np.float32)
    seen = stone[..., 3] > 0
    mean = stone[..., :3][seen].mean(axis=0) if seen.any() else np.array([60.0, 60.0, 70.0])
    pale = tuple(int(min(255, (c * 2.1) + 30)) for c in mean)
    dark = (6, 5, 10)
